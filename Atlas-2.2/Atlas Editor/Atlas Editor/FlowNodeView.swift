...
class FlowNodeView : SKNode
{
    let center:SKSpriteNode
    let volume:SKSpriteNode
    
    init(diameter:Int, tileSize:CGSize)
    {
        ...
        let diameter_x = CGFloat(diameter)*tileSize.width
        let diameter_y = CGFloat(diameter)*tileSize.height
